"""Bounded finite-lexicon search with optional plaintext homophones.

This module is a synthetic-only prototype. It keeps word boundaries fixed and
searches one total cipher-unit to plaintext-letter map. It does not model
nulls, abbreviations, context, or transcription uncertainty.
"""
...
def _normalise_word(word: Word) -> tuple[str, ...]:
    if isinstance(word, str):
        symbols = tuple(word)
    elif isinstance(word, tuple) and all(
        isinstance(symbol, str) and symbol for symbol in word
    ):
        symbols = word
    else:
...
def _candidate_lists(
    ...
    return {
        cipher_word: tuple(
            ...
            if _candidate_compatible(cipher_word, candidate, {}, capacity)
        )
...
def _validate_initial_key(
    ...
    if capacity is not None and any(
        ...
        raise ValueError("initial_key exceeds plaintext-letter capacity")
    return key
...
def _candidate_compatible(
    ...
    existing_counts = Counter(partial_key.values())
    if capacity is not None and any(
        count > capacity for count in existing_counts.values()
    ):
        return False
    ...
    for cipher_symbol, plain_symbol in zip(
        ...
        local_counts[plain_symbol] += 1

    if capacity is not None and any(
        existing_counts[plain_symbol] + count > capacity
        for plain_symbol, count in local_counts.items()
    ):
        return False
    return True
...
def _upper_bound(
    ...
    for cipher_word, weight in ciphertext_counts.items():
        if any(
            _candidate_compatible(cipher_word, candidate, partial_key, capacity)
...
def _complete_key(
    ...
    if capacity is not None and any(count > capacity for count in usage.values()):
        raise ValueError("partial key exceeds plaintext-letter capacity")
    for cipher_symbol in symbol_order:
        if cipher_symbol in key:
            continue
        for plain_symbol in alphabet:
            if capacity is None or usage[plain_symbol] < capacity:
                key[cipher_symbol] = plain_symbol
                usage[plain_symbol] += 1
                break
        else:
            raise ValueError("no feasible total map exists")
    return key
...
def solve_lexicon(
    ...
    *,
    ...
    """Search one total map with bounded plaintext-letter preimages."""
    ...
    if bound_engine == "bitset" and capacity not in (1, 2, None):
        raise ValueError("the bitset engine supports capacities 1, 2, and None")
    ...
    initial = _validate_initial_key(initial_key, cipher_symbols, alphabet, capacity)
    candidates = _candidate_lists(counts, lexicon, capacity)
    candidate_records = [
    ...
    config = {
        ...
        "candidate_constraint": (
            "equal word length, repeated cipher units map equally, and "
            "plaintext preimage capacity is respected"
        ),
        "tie_rule": (
            "lexicographically smallest visited feasible key after maximum score, "
            "using sorted ciphertext-symbol order"
        ),
        "node_budget": node_budget,
        "symbol_order": list(order),
        "initial_key_role": (
            "completed warm-start incumbent only; root search has no fixed assignments"
        ),
        "bound": (
            "sum of weights with at least one compatible candidate, maximized "
            "over the unsearched frontier"
        ),
        "search_exhausted_definition": (
            "The search frontier is empty after traversal or valid pruning. "
            "Direct score certification can occur without traversal. "
            "This field does not mean that every key was enumerated."
        ),
    }
    common = {
        "config": config,
        ...
        "scope_limits": [
            ...
            "A budget stop is certified only when lower_bound equals upper_bound.",
        ],
    }
    ...
    if bound_engine == "bitset":
        ...
        engine = BitsetBound(counts, candidates, alphabet, capacity=capacity)
        bound = engine.bound
        common["bound_metadata"] = engine.metadata
    else:
        def bound(partial_key: Mapping[str, str]) -> int:
            return _upper_bound(partial_key, counts, candidates, capacity)

    best_key = _complete_key(initial, order, alphabet, capacity)
    ...
    while frontier and (budget is None or nodes < budget):
        ...
        completion = _complete_key(key, order, alphabet, capacity)
        completion_score, completion_hit_types = _score_key(
            completion, counts, lexicon
        )
    ...
    score_certified = upper_bound == best_score
    search_exhausted = not frontier
    if search_exhausted:
        status = "exhaustive"
    elif score_certified:
        status = "bound_certified"
    else:
